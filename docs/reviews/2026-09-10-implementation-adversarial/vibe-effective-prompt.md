...
The only tools available are read_file and grep. They are sufficient for this review.
Do not attempt bash, writing a plan file, exit_plan_mode, or any unavailable tool.
Inspect the CURRENT packages under OpenMS4-tests, never implementation/packages
(an earlier staging copy). Preserve reviewer independence and do not read review
outputs. Return your completed review with IDs VIBE-001 etc in your final response.
